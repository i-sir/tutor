...
class TeacherOrderInit extends Base
{

    public $pay_type   = [1 => '微信支付', 2 => '余额支付', 3 => '积分支付', 4 => '支付宝支付', 5 => '组合支付(微信+余额)', 6 => '免费兑换'];//支付类型
    public $is_package = [1 => '是', 2 => '否'];//套餐下单
    public $is_comment = [1 => '是', 2 => '否'];//是否已评价
    public $type       = [1 => '线上', 2 => '线下'];//上课方式


    //后台展示状态列表,统计数量
    public $status_list = [1 => '待付款', 2 => '待接单', 4 => '待上课', 8 => '已完成', 10 => '已取消', 12 => '退款申请', 14 => '退款不通过', 16 => '退款通过'];//状态

    //后台状态,名字,条件
    public $admin_status       = [1 => '待付款', 2 => '待接单', 4 => '待上课', 8 => '已完成', 10 => '已取消', 12 => '退款申请', 14 => '退款不通过', 16 => '退款通过'];//状态
    public $admin_status_where = [1 => [1], 2 => [2], 4 => [4], 8 => [8], 10 => [10], 12 => [12], 14 => [14], 15 => [15], 16 => [16]];

    //前端状态,名字,条件
    public $api_status       = [1 => '待付款', 2 => '待接单', 4 => '待上课', 8 => '已完成', 10 => '已取消', 12 => '退款申请', 14 => '退款不通过', 16 => '退款通过'];//状态
    public $api_status_where = [1 => [1], 2 => [2], 4 => [4], 8 => [8], 10 => [10], 12 => [12], 14 => [14], 15 => [15], 16 => [16]];
    ...
    public function common_item($item = [], $params = [])
    {
        ...
        /** 处理文字描述 **/
        $item['status_name']     = $this->admin_status[$item['status']];//状态
        $item['pay_type_name']   = $this->pay_type[$item['pay_type']];//支付类型
        $item['is_package_name'] = $this->is_package[$item['is_package']];//套餐下单
        $item['is_comment_name'] = $this->is_comment[$item['is_comment']];//是否已评价
    ...
    public function edit_post($params, $where = [])
    {
        ...
        //处理时间格式
        if ($params['comment_time'] && is_string($params['comment_time'])) $params['comment_time'] = strtotime($params['comment_time']);//评价时间
        if ($params['receive_time'] && is_string($params['receive_time'])) $params['receive_time'] = strtotime($params['receive_time']);//接单时间
        if ($params['auto_cancel_receive_time'] && is_string($params['auto_cancel_receive_time'])) $params['auto_cancel_receive_time'] = strtotime($params['auto_cancel_receive_time']);//自动取消接单时间
        if ($params['cancel_receive_time'] && is_string($params['cancel_receive_time'])) $params['cancel_receive_time'] = strtotime($params['cancel_receive_time']);//取消接单时间
        ...
        if ($params['send_time'] && is_string($params['send_time'])) $params['send_time'] = strtotime($params['send_time']);//发货时间
        if ($params['take_delivery_time'] && is_string($params['take_delivery_time'])) $params['take_delivery_time'] = strtotime($params['take_delivery_time']);//收货时间
        ...
        if (!empty($where)) {
            ...
            $result                = $TeacherOrderModel->where($where)->strict(false)->update($params);
            //if ($result) $result = $item["id"];
        } elseif (!empty($params["id"])) {
            //如传入id,根据id编辑数据
            $params["update_time"] = time();
            $result                = $TeacherOrderModel->where("id", "=", $params["id"])->strict(false)->update($params);
            //if($result) $result = $item["id"];
        } else {
            //无更新条件则添加数据
            $params["create_time"] = time();
            $result                = $TeacherOrderModel->strict(false)->insert($params, true);
        }
    ...
    public function edit_post_two($params, $where = [])
    {
        ...
        if (!empty($where)) {
            ...
            //无更新条件则添加数据
            $params["create_time"] = time();
            $result                = $TeacherOrderModel->strict(false)->insert($params);
        }
    ...
    public function delete_post($id, $type = 1, $params = [])
    {
        ...
        if ($type == 1) $result = $TeacherOrderModel->destroy($id);//软删除 数据表字段必须有delete_time
        if ($type == 2) $result = $TeacherOrderModel->destroy($id, true);//真实删除
    ...
    public function batch_post($id, $params = [])
    {
        ...
        $params["update_time"] = time();
        $result                = $TeacherOrderModel->where($where)->strict(false)->update($params);//修改状态
    ...
    public function list_order_post($list_order, $params = [])
    {
        ...
        foreach ($list_order as $k => $v) {
            $where   = [];
            $where[] = ["id", "=", $k];
            $result  = $TeacherOrderModel->where($where)->strict(false)->update(["list_order" => $v, "update_time" => time()]);//排序
        }
    ...
    public function export_excel($where = [], $params = [])
    {
        ...
        $result = $TeacherOrderInit->get_list($where, $params);
